...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None):
        ...
        if self.classes is None and self.pictures is None:
            ...
            features_filename = os.path.join(f'/ibex/user/thagafhh/data/train_image_latent_512.pt') if self.train else os.path.join(f'/ibex/user/thagafhh/data/test_image_latent_512.pt')
            if os.path.exists(features_filename) :
                saved_features = torch.load(features_filename)
                if self.train:
                    self.text_features = torch.load('/ibex/user/thagafhh/data/ViT-H-14_features_train.pt')
                    self.text_features = self.text_features['text_features']
                else: 
                    self.text_features = torch.load('/ibex/user/thagafhh/data/ViT-H-14_features_test.pt')
                    self.text_features = self.text_features['text_features']
    ...
    def __getitem__(self, index):
        ...
        # 2. Dxgdgerive the text_index directly from the label.
