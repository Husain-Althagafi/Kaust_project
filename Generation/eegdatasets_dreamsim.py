...
print('preprocess of vl:', preprocess_train)

model_type = 'dreamsim_ViT-H-14'
cache_path = "/ibex/user/thagafhh/model_cache/dreamsim"
dreamsim_weights_path = '/ibex/user/thagafhh/data/dreamsim_data/logs/finetune__dino-like_clip_h14_mlp_head_open_clip_vith14_cls_lora_lr_0.001_batchsize_16_wd_0.0_hiddensize_512_margin_0.01_lorar_8_loraalpha_0.1_loradropout_0.1/dreamsim_training/amv064co/checkpoints/epoch=09.ckpt'
exported_weights_path = "/ibex/user/thagafhh/data/dreamsim_data/converted_weights/exported_native_weights_v3_final.pth"

print(f'------------Loading {model_type}------------')

from dreamsim_loader import load_human_aligned_teacher

human_aligned_vlmodel = load_human_aligned_teacher(exported_weights_path)
human_aligned_vlmodel.to(device)
human_aligned_vlmodel.eval()
...
class EEGDataset():
    ...
    def ImageEncoder(self,images):
        ...
        for i in tqdm(range(0, len(images), batch_size), desc="Encoding Images", unit="batch"):
            ...
            # image_inputs = torch.stack([dreamsim_preprocess(Image.open(img).convert("RGB")) for img in batch_images]).to(device)
            ...
            with torch.no_grad():
                ...
                batch_image_features = human_aligned_vlmodel(image_inputs)
