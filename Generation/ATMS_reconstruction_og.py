...
import tqdm
from eegdatasets_og import EEGDataset
    ...
    for epoch in range(config.epochs):
        ...
        if (epoch +1) % 5 == 0:                    
            ...
            if config.insubject==True:       
                os.makedirs(f"/ibex/user/thagafhh/data/models/contrast/ogtrain/{config.encoder_type}/{sub}/{current_time}", exist_ok=True)             
                file_path = f"/ibex/user/thagafhh/data/models/contrast/ogtrain/{config.encoder_type}/{sub}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)                     
            else:                
                os.makedirs(f"/ibex/user/thagafhh/data/models/contrast/across/ogtrain/{config.encoder_type}/{current_time}", exist_ok=True)             
                file_path = f"/ibex/user/thagafhh/data/models/contrast/across/ogtrain/{config.encoder_type}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)
...
def main():
    ...
    parser.add_argument('--name', type=str, default="basic run", help='Experiment name')
