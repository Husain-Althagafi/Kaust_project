...
dreamsim_model, dreamsim_preprocess = dreamsim(pretrained=True, cache_dir=cache_path, device=device)
...
class EEGDataset():
    ...
    def ImageEncoder(self,images):
        ...
        for i in tqdm(range(0, len(images), batch_size), desc="Encoding Images", unit="batch"):
            ...
            image_inputs = torch.cat([dreamsim_preprocess(Image.open(img).convert("RGB")) for img in batch_images]).to(device)


            with torch.no_grad():
                # print("test for image_inputs:", image_inputs.shape)
                batch_image_features = dreamsim_model.embed(image_inputs)
